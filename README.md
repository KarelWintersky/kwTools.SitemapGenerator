...
; YMD - дата в формате Y-m-d (2004-12-23)
...
Собирается при помощи `sh`-скрипта `build_minified.sh`. Результирующий файл будет называться `sitemap_generator.php`. Ему, разумеется, для работы
