...
В папке dest есть пример необходимого набора файлов:
- `config.sitemap+db.ini.example` - пример конфигурационного файла
- `data.countries.txt` - текстовый файл со списком страниц
- `data.staticpages.txt` - текстовый файл со списком статических страниц
- `sitemap_generator.php` - скрипт для запуска (

## Установка

Все исходники лежат в корневом каталоге и собираются в `sitemap_generator.php` при помощи build-скрипта.

```
/bin/bash ./build.minified.sh
```

После этого мы можем сделать так:
```
chmod +x ./dest/sitemap_generator.php
mv ./dest/sitemap_generator.php /usr/local/bin
```
И запускать его без указания пути к интерпретатору.
...
Все настройки доступа к БД и инструкции по построению сайтмэпов задаются в ini-файлах. Предполагается, что конфиг-файлы недоступны посторонним,
но я все равно рекомендую создать для генерации сайтмэпа отдельного пользователя исключительно с правом SELECT и использовать в файлах конфигурации
 эти значения.
...
; суффикс для секции с настройками подключения. Это обязательная опция и она не может быть пустой.
db_section_suffix = 'DATABASE'
```

### Секция настроек подключения к БД

Имя этой секции: `___GLOBAL_SETTINGS:<suffix>___`, суффикс описан выше. Это обязательная секция, без неё всё сломается.
...
На моём домашнем сервере (Gentoo/4.2.15 на GA-N3150N-D3V, CPU Celeron™ N3150 (1.6 GHz), PHP 7.0.1) для тестовой БД с ~250 тыс. записей
файлы сайтмэпа создаются около 15 секунд. При этом пиковое потребление памяти составляет ~80 Мб.
